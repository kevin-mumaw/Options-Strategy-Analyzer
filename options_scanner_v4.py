...
"pct_ma50": pct50,
"pct_ma200": pct200,
"detail": f"{config['regime_symbol']} {price} | MA50 {ma50} ({pct50:+.1f}%) | })
...
breakdown["support"] = 0
reasons.append(f"✗ Support [0/1] Extended {pct_from_ma50:+.1f}% above MA50 — wait for else:
breakdown["support"] = 0
...
breakdown["macd"] = 1
reasons.append(
f"✓ MACD [1/1] Bullish — MACD {macd['macd']:.3f} above signal {macd['signal']:.)
else:
breakdown["macd"] = 0
reasons.append(
f"✗ MACD [0/1] Bearish — MACD {macd['macd']:.3f} below signal {macd['signal']:.)
else:
breakdown["macd"] = 0
...
reasons.append(
f"✓ BB [1/1] Near lower band — oversold bounce (BB% {bollinger['pct_b']:.)
...
if best_dte is None or abs(dte - config["target_dte"]) < abs(best_dte - config["best_exp = exp
best_dte = dte
...
"warning": f"⚠ Earnings in {days} day{'s' if days != 1 else ''} ({})
return result
elif days > warn_days:
...
hard_fail = f"Overbought (RSI {rsi:.0f}) AND extended (+{pct_ma50:.1f}% above MA50) elif rsi >= 75:
...
print(f" Earnings: {ed} ({days} days away — outside {config['earnings_warn_# Option
print(f"\n OPTION:")
